...
# This is version 0.4.0 of QuAPPro, a shiny app for 
                                          ...
                                          fluidRow(
                                            ...
                                            column(6, style = "padding-top:20px",
                                                   div( id = "select_area_wrapper",
                                                        selectizeInput("select_area", "Select a name", options = list(create=TRUE, plugins = list('restore_on_backspace')),
                      ...
                      wellPanel( 
                        ...
                        fluidRow(
                          ...
                          column(4, style = "border-right: 1px dotted #d3d3d3;border-bottom: 1px dotted #d3d3d3",
                                 ...
                                 fluidRow(
                                   column(6, style = "padding-top:3px;padding-bottom:15px",
             ...
             fluidRow(
               ...
               column(10,
                      tags$div(
                        ...
                        htmltools::includeMarkdown("QuAPPro_v0-4-0_manual.Rmd")
                      )
               ),
               column(1)
             ) 
    ...
    tabPanel(tags$strong("Release Notes", style = "color:blue"), icon = icon("info", style = "color:blue"), 
             fluidRow(
               ...
               column(10, 
                      tags$div(
                        style = "font-size: 18px;",
                        htmltools::includeMarkdown("QuAPPro_v0-4-0_releaseNotes.Rmd"))
                      ), 
               column(1)
             ) 
    ),
...
server <- function(input, output, session) {
  ...
  import <- function(path){
    ...
    for(i in setdiff(names(val), names(forExport) ) ){
      val[[i]] <- NULL
    }
    ...
    updateSelectInput(session, "input_table_barplot", choices = unique(names(val$sum_areas) ), selected = val$selected_input_table_barplot)
    updateSelectInput(session, "normalize_barplot", choices = c("None" , unique(names(val$sum_areas) ) ), selected = val$normalize_barplot )
    
    updateSelectInput(session, "selected_cond1", choices = val$cond1, selected = val$selected_cond1 )
    updateSelectInput(session, "selected_cond2", choices = val$cond2, selected = val$selected_cond2 )
    ...
    # This value signals that colors originate from a re-import of previous data
    # and therefore, the current palette should not overwrite the colors set by the user.
    if(val$color_palette != "dark_palette"){
      val$reimport_colors <- TRUE
    }
    
    # This value signals that the settings for deconvolution originate from a re-import of previous data
    # and therefore, the re-imported deconvolutions should not be deleted.
    val$reloaded_decon <- TRUE
    
    if(isTruthy(val$select)){
      updateSelectInput(session, "select", selected = val$select)
    }
    
    if(isTruthy(val$select2)){
      updateSelectInput(session, "select2", selected = val$select)
    }
    
    if(isTruthy(val$select_peak)){
      updateSelectInput(session, "select_peak", selected = val$select_peak)
    }
  }
  ...
  read_data <- function(){
    ...
    for(i in 1:length(new_names)){
      ...
      # If the data in the selected column is not numeric,
      # thousand separators might be the reason,
      # and need to be removed.
      # If a comma is used as decimal separator,
      # it has to be replaced by a point 
      # before converting the vector to mode numeric.
      if( !is.numeric( UV ) ){
        ts <- setdiff( c(".", ","), input$dec)
        no_ts <- gsub(ts, "", UV, fixed = T)
        if(input$dec == ","){
          no_ts <- gsub(",", ".", no_ts, fixed = T)
        }
        UV <- as.numeric(no_ts)
      }
  ...
  observeEvent(input$input_data, {
    ...
    if( length(input$input_data$name) > 1 & any(grepl("RData", input$input_data$name ) ) )
    {
      ...
      if( length(input$input_data$name) == 1 & any( grepl("RData", input$input_data$name ) ) )
      {
        loading_attempt <- try( import(input$input_data$datapath), silent = T )
      }else{
  ...
  observeEvent(input$smooth_pol, {
    ...
    # In case the assignment was triggered by a reloading of data
    # or by changing the selected profile:
    val$reloaded_decon <- NULL
  })
  ...
  plot_singleFl <- function(cex_lab, cex_axis, lwd, mgp2, lwd_box){
    if(lost_num_fl() == 1 ){
      ...
      ylab <- paste("Fluo. (/ ", lost_num_fl(), ")", sep = "")
    }
    ...
    if(isTruthy(val$area_starts[[input$select_area]][input$select]) & isTruthy(val$area_ends[[input$select_area]][input$select]) & input$green_lines & isTruthy(val$baseline[input$select])){
      ...
      if(isTruthy(val$fl_control_baseline[[input$select_area]][input$select])){
        polygon(c(x_first, val$xvalues[[input$select]][ x_first : x_last ], x_last),
                ...
                col = adjustcolor("#c7e9c0", alpha = 0.9), border = "black", lwd = 0.8*lwd)
      }
    }
    if(input$start_end){
    ...
    if(input$red_lines2){
      abline(h = val$baseline_fl[input$select], col = "grey", lty = 2, lwd = lwd)
    }
    if(input$red_lines){
  ...
  plot_singlePol <-function(cex_lab, cex_axis, lwd, mgp1, mgp2, lwd_box){
    ...
    if(lost_num_x() == 1 ){
      ...
      xlab <- paste("Data points (/ ", lost_num_x(), ")", sep = "")
    }
    
    if(lost_num_pol() == 1 ){
      ylab <- "UV abs."
    }else{
      ylab <- paste("UV abs. (/ ", lost_num_pol(), ")", sep = "")
    }
  ...
  observeEvent(input$select, {
    val$buttons = 10
    # Store the selected profile for re-import
  ...
  observeEvent(input$click$x, {
    ...
    # Usually, a point on the profile is selected as baseline.
    # If the user clicks outside into the plot to the left or the right of the profile,
    # the selected y-value is directly used as baseline.
    if(val$buttons == 2){
      if( round(input$click$x) %in% val$xvalues[[input$select]] ) {
        val$baseline[input$select] <- polysomes()[input$click$x]
      }else{
        val$baseline[input$select] <- input$click$y
      }
    }
    ...
    if(val$buttons == 5){
      ...
      if( round(input$click$x) %in% val$xvalues[[input$select]] ) {
        ...
        val$baseline_fl[input$select] <- input$click$y
      }
    }
  ...
  observeEvent(input$quantify_area,{
    ...
    val$sum_areas[[area_name]][input$select] <-
      sum( polysomes()[sort(x_first:x_last)] - val$baseline[input$select], na.rm = T )
    
    if(input$show_fl & isTruthy(val$baseline_fl[input$select]) ){
      area_fluo_name <- paste(input$select_area, "_fluo_area")
      val$sum_areas[[area_fluo_name]][input$select] <- 
        sum( fluorescence()[sort(x_first:x_last)] - val$baseline_fl[input$select], na.rm = T )
    }
  ...
  observe({
    ...
    file_names <- names(val$sum_areas[[ a ]])
    
    df_quant <- data.frame(File = file_names, val$sum_areas[[ a ]], check.names = FALSE)
    colnames(df_quant)[2] <- a

    for(a in areas_with_quant[-1]){
      df_quant_new <- data.frame(File = names(val$sum_areas[[ a ]]), val$sum_areas[[ a ]], check.names = FALSE)
      colnames(df_quant_new)[2] <- a
      df_quant <- merge(df_quant, df_quant_new, by = "File", all = T)
    }
    
    all_files_with_quant <- df_quant$File
    basenames <- gsub(".csv|.txt|.pks|.tsv|.asc", "", all_files_with_quant)
    
    names(basenames) <- all_files_with_quant
    if(isTruthy(val$legend_names) ){
      is_in_legend <- intersect( all_files_with_quant, names(val$legend_names) ) 
      basenames[is_in_legend] <- unlist( val$legend_names[is_in_legend] )
    }
    val$df_quant <- data.frame(Name = basenames[ df_quant$File ], df_quant, check.names = FALSE)
  })
  ...
  norm_factor_x <- reactive({
    ...
    if( all(val$files_to_plot %in% files_with_total) & input$normalize_length ){
      all_lengths <- abs( round(val$area_ends[["Total"]][val$files_to_plot] - val$area_starts[["Total"]][val$files_to_plot] ) )
      ref_length <- max(all_lengths)
      dummy <- abs( round(val$area_ends[["Total"]][val$files_to_plot] - val$area_starts[["Total"]][val$files_to_plot] ) / ref_length )
    }else{
      dummy <- rep( 1, length(val$files_to_plot) )
    }
    names(dummy) <- val$files_to_plot
  ...
  # This should not happen when the selected color palette is changed
  # due to a re-import of .RData.
  observeEvent(input$color_palette, {
    if(!isTruthy(val$reimport_colors)){
      val$colors_collected = list()
    }else{
      val$reimport_colors <- NULL
    }
  }
  )
  ...
  observeEvent(input$select_alignment, {
    ...
    updateTextInput(session, "new_name", value = val$legend_names[[input$select_alignment]])
  })
  ...
  plot_alignedPol <- function(cex_lab, cex_axis, lwd, 
    ...
    if(lost_num_al_pol() == 1 ){
      ...
      ylab <- paste("UV abs. (/ ", lost_num_al_pol(), ")", sep = "")
    }
    
    if(lost_num_al_Index() == 1 ){
      xlab <- "Relative position"
    }else{
      xlab <- paste("Relative position (/ ", lost_num_al_Index(), ")", sep = "")
    }
  ...
  plot_alignedFluo <- function(cex_lab, cex_axis, lwd, 
    ...
    if(lost_num_al_fl() == 1 ){
      ...
      ylab <- paste("Fluo. (/ ", lost_num_al_fl(), ")", sep = "")
    }
  ...
  plot_2nd_deriv <-function(cex_lab, cex_axis, lwd, mgp2, lwd_box){
    plot(val$xvalues[[input$select2]][-c(1:2)], second_deriv_smoothed(), type = "l", lwd = lwd, xaxt = "n", col = "black", 
         las = 1, ylab = "2nd derivative", mgp = mgp2, xlab = "", yaxt = "n", cex.lab = cex_lab,
         xlim = c(xmin_d(), xmax_d()), bty = "n" )
    axis(2, at = 0, labels = 0, cex.axis = cex_axis, las = 1, lwd = 0.8*lwd_box)
    box(lwd = lwd_box)
    a <- axTicks(2)
  }
  
  # Plot the individual peaks with the positions, heights 
  # and standard deviations as determined by deconvolution.
  ...
  plot_peaks <- function(lwd){
    ...
    for(i in inactive_peaks){
      ...
      points(x, curve, type = "l", lwd = lwd, col = "azure4")
    ...
    if(!is.null(val$active_peak[[input$select2]])){
      ...
      polygon( c(val$xvalues[[input$select2]][1], val$xvalues[[input$select2]], tail(val$xvalues[[input$select2]], 1) ), 
               c(0, curve, 0), col = adjustcolor("#c7e9c0", alpha = 0.9),
               border = NA)
  ...
  plot_UV_deconv <-function(cex_lab, cex_axis, lwd, 
    ...
    if(lost_num_pol_d() == 1 ){
      ...
      ylab <- paste("UV abs. (/ ", lost_num_pol_d(), ")", sep = "")
    }
    
    if(lost_num_x_d() == 1 ){
      xlab <- "Data Points"
    }else{
      xlab <- paste("Data points (/ ", lost_num_x_d(), ")", sep = "")
    }
  ...
  plot_deconv <- function(cex_lab, cex_axis, lwd, mgp1, mgp2, mar_factor, lwd_box){
    if(!is.null(val$xvalues[[input$select2]])){
      if(input$show_deriv){
        ...
        abline( v = second_deriv_min(), col = "grey", lty = 2, lwd = 0.5*lwd )
        par(mar = c(5, 5, 0, 0.5)*mar_factor)
  ...
  show_note <- function(){
    ...
    legend("center", legend = "Deconvolution requires a profile with a baseline in the Alignment/Areas tab.",
           bty = "n", cex = 2)
  }
  ...
  # so that deconvolution is always performed between two putative peaks.
  observeEvent(input$click_deconv$x, {
    ...
    if(val$buttons == 6){
      ...
      potential_starts <- c()
      if(input$helper_functions_decon){
        minmax <- c( all_min(smooth.spline(val$xvalues[[input$select2]], y, NULL)$y, 5),
                     all_max(smooth.spline(val$xvalues[[input$select2]], y, NULL)$y, 5) )
        potential_starts <- c(potential_starts, minmax)
      }
      if(input$inflection_functions_decon){
        infl <- all_inflections(smooth.spline(val$xvalues[[input$select2]], y, NULL)$y, 5)
        potential_starts <- c(potential_starts, infl)
      }
      if(input$derivMin_functions_decon){
        derivMin <- second_deriv_min()
        potential_starts <- c(potential_starts, derivMin)
      }
      
      if(!input$helper_functions_decon & !input$inflection_functions_decon & !input$derivMin_functions_decon){  
        val$deconv_start[[input$select2]] <- a
      }else{
        val$deconv_start[[input$select2]] <- potential_starts[ order( abs(potential_starts -  a), decreasing = F )[1] ]
      }
    }
    ...
    if(val$buttons == 7){
      ...
      if(input$helper_functions_decon){
        minmax <- c( all_min(smooth.spline(val$xvalues[[input$select2]], y, NULL)$y, 5),
                     all_max(smooth.spline(val$xvalues[[input$select2]], y, NULL)$y, 5) )
        potential_ends <- c(potential_ends, minmax)
      }
      if(input$inflection_functions_decon){
        infl <- all_inflections(smooth.spline(val$xvalues[[input$select2]], y, NULL)$y, 5)
        potential_ends <- c(potential_ends, infl)
      }
      if(input$derivMin_functions_decon){
        derivMin <- second_deriv_min()
        potential_ends <- c(potential_ends, derivMin)
      }
      
      if(!input$helper_functions_decon & !input$inflection_functions_decon & !input$derivMin_functions_decon){  
        val$deconv_end[[input$select2]] <- a
      }else{
        val$deconv_end[[input$select2]] <- potential_ends[ order( abs(potential_ends -  a), decreasing = F )[1] ]
      }
    }
    
    # When deconvolution was successful, the table with deconvolution parameters exists.
    # The user can now click into the plot and select a peak for quantification,
    # unless selection of start or end for quantification was selected.
    if(val$buttons != 6 & val$buttons != 7 & isTruthy(val$decon_param_df[[input$select2]]) ){
      a <- closest_point(input$click_deconv$x, input$click_deconv$y, val$xvalues[[input$select2]], yvalue_deconv() )
  ...
  # the previous deconvolution is deleted.
  observeEvent(val$resolution[[input$select2]], {
    if(length(val$peak_pos[[input$select2]]) > 0 & !isTruthy(val$reloaded_decon) ){
      ...
      val$mse[[input$select2]] <- NULL
    }
  })
  
  # When the smoothing parameter of the second derivative
  # is changed by the user for the selected profile,
  # the previous deconvolution is deleted.
  ...
  # When the smoothing parameter of the UV profile
  # is changed by the user for the selected profile in the main tab,
  # the previous deconvolution is deleted.
  observeEvent(val$smooth_pol[[input$select2]], {
    if(length(val$peak_pos[[input$select2]]) > 0 & !isTruthy(val$reloaded_decon) ){
      val$peak_pos[[input$select2]] <- NULL
      val$peak_sd[[input$select2]] <- NULL
      val$peak_height[[input$select2]] <- NULL
  ...
  # Resolution and smoothing parameters are updated when another profile
  # is selected.
  observeEvent( input$select2, {
    val$select2 <- input$select2
  ...
  observeEvent(input$decon_go, {
    ...
    mu <- intersect(second_deriv_min(), sort(from:to) )
    height <- y[mu]
    ...
    df <- data.frame(x = x[sort(from:to)], y = y[sort(from:to)])
    ...
    if( any( class(decon_attempt) == "try-error") ){
      showNotification("Deconvolution failed. For more information, refer to the manual.",
                       duration = 5, type = "error")
      val$peak_pos[[input$select2]] <- NULL
      val$peak_sd[[input$select2]] <- NULL
      val$peak_height[[input$select2]] <- NULL
      ...
      # If deconvolution was successful, 
      # the parameters for deconvolution (smoothing, resolution) and
      # the peak information is stored for the selected profile.
      ...
      val$mse[[input$select2]] <- sum( ( df$y - val$model[[input$select2]][sort(from:to)] )^2 )/length(df$y)
      val$decon_param_df[[input$select2]] <- data.frame(Position = unlist(val$peak_pos[[input$select2]]), 
                                                        ...
                                                        Height = unlist(val$peak_height[[input$select2]])
      )
      val$smooth_pol_decon[[input$select2]] <- input$smooth_pol_decon
      val$resolution[[input$select2]] <- input$resolution
      # To allow selection of peaks for quantification after deconvolution, 
      # the value of the button needs to be set to a value different from 6 or 7:
      val$buttons <- 10
    }
  })
  ...
  # The currently selected peak is stored for restoring when a previous analysis is loaded as .RData.
  observeEvent(input$select_peak, {
    val$current_peak <- input$select_peak
  })
  
  # The mean squared error of the model is displayed as text.
  output$MSE <- renderText(
    if(isTruthy(val$mse[[input$select2]])){
      paste("MSE =", round( val$mse[[input$select2]], digits = 3 ) ) 
    }
  )
  ...
  # When an area or peak was quantified, the name is added to the two menus
  # for selecting the feature of interest and the feature to be used for normalization.
  # The selected value is not changed:
  observeEvent(names(val$sum_areas), {
    updateSelectInput(session, "input_table_barplot", choices = unique(names(val$sum_areas) ), selected = val$selected_input_table_barplot )
    updateSelectInput(session, "normalize_barplot", choices = c("None" , unique(names(val$sum_areas) ) ), selected = val$normalize_barplot )
  })
  
  # The selected feature of interest is stored for re-import.
  observeEvent(input$input_table_barplot, {
    val$selected_input_table_barplot <- input$input_table_barplot
  })
  
  # The selected feature for normalization is stored for re-import.
  observeEvent(input$normalize_barplot, {
    val$normalize_barplot <- input$normalize_barplot
  })
  
  # An empty table is created, which is used for assigning conditions to profiles. 
  val$barplot_table <- data.frame(Name = "",
                                  Values = "",
                                  Condition1 = "",
                                  Condition2 = "")
  
 # An editable table is generated so that users can enter the conditions of their profiles.
  output$barplot_table <- renderDT(
    val$barplot_table, 
  ...
  # The values in the table are updated when the selected feature of interest, 
  # the feature for normalization or the quantified values (val$df_quant) are changed.   
  observeEvent(c(val$selected_input_table_barplot, val$normalize_barplot, val$df_quant), {
    req(val$df_quant)
    files <- val$df_quant[, "File"]
    file_names <- val$df_quant[, "Name"]
    file_number <- length(file_names)
    val$barplot_table <- data.frame(Name = rep("", file_number),
                                    Values = rep("", file_number),
                                    Condition1 = rep("", file_number),
                                    Condition2 = rep("", file_number) )
    val$barplot_table[ , 1] <- file_names
    rownames(val$barplot_table) <- val$df_quant[, "File"]
    
    barplot_input_name <- gsub("_area|_decon", "", val$selected_input_table_barplot)
    barplot_normalize_name <- gsub("_area|_decon", "", val$normalize_barplot)
    if(val$normalize_barplot != "None" & val$selected_input_table_barplot != ""){
      val$barplot_table[ , 2] <- round(val$df_quant[,val$selected_input_table_barplot] / val$df_quant[,val$normalize_barplot], 2)
      val$barplot_ylab <- paste(barplot_input_name, barplot_normalize_name, sep = " / ")
    }
    if(val$normalize_barplot == "None" & val$selected_input_table_barplot != ""){
      val$barplot_table[ , 2] <- round(val$df_quant[,val$selected_input_table_barplot], 2)
      val$barplot_ylab <- barplot_input_name
    }
    
    if(isTruthy(val$cond1_files)){
      cond1_file_names <- names(val$cond1_files)
      pos <- match(cond1_file_names , files)
      val$barplot_table[pos, 3] <- val$cond1_files
      if(!all(files %in% cond1_file_names)){
        val$barplot_table[!(files %in% cond1_file_names), 3] <- ""
      }
    }
    if(isTruthy(val$cond2_files)){
      cond2_file_names <- names(val$cond2_files)
      pos <- match(cond2_file_names , files)
      val$barplot_table[pos, 4] <- val$cond2_files
      if(!all(files %in% cond2_file_names)){
        val$barplot_table[!(files %in% cond2_file_names), 4] <- ""
      }
    }
  })
  
  # When the user assigns conditions to the files, 
  # the table is updated:
  observeEvent(input$barplot_table_cell_edit, {
    row  <- input$barplot_table_cell_edit$row
    col <- input$barplot_table_cell_edit$col
    val$barplot_table[row, col] <- input$barplot_table_cell_edit$value
    # Store conditions for the files to re-construct the table 
    # if additional quantifications are added:
    if(col == 3){
      val$cond1_files <- val$barplot_table[ , 3]
      names(val$cond1_files) <- row.names(val$barplot_table)
    }
    if(col == 4){
      val$cond2_files <- val$barplot_table[ , 4]
      names(val$cond2_files) <- row.names(val$barplot_table)
    }
    
  })
  
  # When the table with the values for the bar plot is updated,
  # the choices in the menu of conditions for setting the colors 
  # might also have to be updated.
  # Empty strings in the condition fields are ignored.
  observeEvent(val$barplot_table, {
    current_cond1 <- setdiff( unique(val$barplot_table[ , 3] ), "")
    current_cond2 <- setdiff( unique(val$barplot_table[ , 4] ), "")
    if( !all(current_cond1 %in% val$cond1 ) | !all(val$cond1 %in% current_cond1 ) ){
      val$cond1 <- current_cond1
      
      updateSelectInput(session, "selected_cond1", choices = val$cond1 )

      cond1 <- unique(val$barplot_table[ , 3] )
      number_of_cond1 <- length( cond1 )
      
      # For the bars, grey is set as default color
      dummy <- list()
      dummy[cond1] <- rep("#999999", number_of_cond1)
      val$color_cond1 <- dummy
      
      # For the data points, black is set as default color
      dummy <- list()
      dummy[cond1] <- rep("black", number_of_cond1)
      val$color_dots_cond1 <- dummy
      
      # The new color palette is shown in the color input fields for the selected condition:
      updateColourInput(session, "color_cond1", value = val$color_cond1[[input$selected_cond1]])
      updateColourInput(session, "color_dots_cond1", value = val$color_dots_cond1[[input$selected_cond1]])
      
    }
    if( !all(current_cond2 %in% val$cond2 ) ){
      val$cond2 <- current_cond2
      
      # A default color palette is applied until the user 
      # has changed the colors for the individual conditions.
      # When new conditions are added to condition 2,
      # a new palette is generated, so that previous color choices
      # are overwritten.
      # The colors have to be stored as a named list (necessary for JSON object output).
      observeEvent( val$cond2, { 
        updateSelectInput(session, "selected_cond2", choices = val$cond2 )
        
        dummy <- list()
        pal <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
        cond2 <- unique(val$barplot_table[ , 4] )
        number_of_cond2 <- length( cond2 )
        
        # If there are more conditions than colors in the palette,
        # the colors have to be repeated repeated.
        # Then, we select as many colors as we have conditions:
        dummy[cond2] <- rep(pal, ceiling( number_of_cond2/length(pal ) ) )[1:number_of_cond2]
        val$color_cond2 <- dummy
        
        # For the data points, black is set as default color
        dummy <- list()
        dummy[cond2] <- rep("black", number_of_cond2)
        val$color_dots_cond2 <- dummy
        
        # The new color palette is shown in the color input fields for the selected condition:
        updateColourInput(session, "color_cond2", value = val$barplot_color_palette[[input$selected_cond2]])
        updateColourInput(session, "color_dots_cond2", value = val$color_barplot_dot_cond2[[input$selected_cond2]])
      })
    }
  })
  
  # When the selected condition 1 is changed, the selection is stored for re-import.
  observeEvent(input$selected_cond1, {
    val$selected_cond1 <- input$selected_cond1
  })
  
  # When the selected condition 2 is changed, the selection is stored for re-import.
  observeEvent(input$selected_cond2, {
    val$selected_cond2 <- input$selected_cond2
  })
  
  # When the selected condition 1 is changed by the user,
  # the selected colors for bars and data points are updated:
  observeEvent( input$selected_cond1, {
    updateColourInput(session, "color_cond1", value = val$color_cond1[[input$selected_cond1]])
    updateColourInput(session, "color_dots_cond1", value = val$color_dots_cond1[[input$selected_cond1]])
  })
  
  # The user can change the color palette for the bars in condition 1.
  observeEvent(input$color_cond1,  {
    req(val$color_cond1)
    val$color_cond1[[input$selected_cond1]] <- input$color_cond1
  })
  
  # The user can change the color palette for the data points in condition 1.
  observeEvent(input$color_dots_cond1,  {
    req(val$color_dots_cond1)
    val$color_dots_cond1[[input$selected_cond1]] <- input$color_dots_cond1
  })
  
  # When the selected condition 2 is changed by the user,
  # the selected colors for bars and data points are updated:
  observeEvent( input$selected_cond2, {
    updateColourInput(session, "color_cond2", value = val$color_cond2[[input$selected_cond2]])
    updateColourInput(session, "color_dots_cond2", value = val$color_dots_cond2[[input$selected_cond2]])
  })
  
  # The user can change the color palette for the bars in condition 2.
  observeEvent(input$color_cond2,  {
    req(val$color_cond2)
    val$color_cond2[[input$selected_cond2]] <- input$color_cond2
  })
  
  # The user can change the color palette for the data points in condition 2.
  observeEvent(input$color_dots_cond2,  {
    req(val$color_dots_cond2)
    val$color_dots_cond2[[input$selected_cond2]] <- input$color_dots_cond2
  })
  
  # Generate a bar plot from the data using ggplot2:
  barplot <- function(bs, ps){ 
    req( any(val$barplot_table$Values != "") )
    req( any(val$barplot_table$Condition1 != "") | any(val$barplot_table$Condition2 != "") )
    df <- val$barplot_table
    
    # If there is no condition 2, the column is removed
    # and a barplot with only one bar per condition 1 is generated.
    if (all(df$Condition2 == "")) {
      df$Condition2 <- NULL 
      
      # Filter data frame so that empty conditions and values are removed:
      df <- df[ df$Condition1 != "" & df$Values != "" & !is.na(df$Condition1) & !is.na(df$Values), ]
      
      # Calculating mean and SD per condition 1:
      my_stat <- df %>%  
        group_by(Condition1) %>%
        summarise(mean = mean(Values),
                  sd = sd(Values))
      
      # Merging mean and SD with the data frame,
      # so that the individual values and their means 
      # and SDs are available for the bar plot:
      df <- merge(df, my_stat)
      
      # GGplot
      g <- ggplot(data = df, aes(x = Condition1, y = Values, fill = Condition1, colour = Condition1) ) +
        ...
        theme(axis.text = element_text(size = bs) ) +
        theme(axis.title.x = element_blank(), legend.position = "none",
              axis.line = element_line(size = 0.2 ),
              panel.border = element_rect(color = "black", fill = NA, size = 0.2) ) +
        ylab(val$barplot_ylab) +
        scale_fill_manual("Condition", values = unlist( val$color_cond1 ) ) +
        scale_colour_manual("Condition", values = unlist(val$color_dots_cond1 ) )
      
      plot_points <- any(duplicated(df$Condition1))
      if(plot_points){
        g + geom_quasirandom(cex = ps, method = "quasirandom", dodge.width = 0.9, width = 0.3 )
      }else{
        g
      }
      
    }else{
      # If there are entries in condition 2,
      # a bar plot with two grouping factors is generated.
      
      # Filter data frame so that empty conditions and values are removed:
      df <- df[ df$Condition2 != "" & df$Values != "" & !is.na(df$Condition2) & !is.na(df$Values), ]
      ...
      g <- ggplot(data = df, aes(x = Condition1, y = Values, fill = Condition2, colour = Condition2) ) +
        geom_bar(stat = "summary", 
                 position = position_dodge(width = 0.9),
                 fun = "mean",
                 colour = "black", size = 0.2) +
        ...
        theme(axis.text = element_text(size = bs), axis.line = element_line(size = 0.2 ),
              panel.border = element_rect(color = "black", fill = NA, size = 0.2) ) +
        ylab(val$barplot_ylab) +
        scale_fill_manual("Condition", values = unlist( val$color_cond2 ) ) +
        scale_colour_manual("Condition", values = unlist( val$color_dots_cond2 ) )
      
      plot_points <- any( duplicated( paste( df$Condition1, df$Condition2 ) ) )
      if(plot_points){
        g + geom_quasirandom(cex = ps, method = "quasirandom", dodge.width = 0.9 )
      }else{
        g
      }
    }
  ...
  output$downloadPlot_single <- downloadHandler(
    filename = function(){ 
      gsub(".csv|.txt|.pks|.tsv|.asc", "_QuAPPro.pdf", as.character(input$select) )
    },
    content = function(file) {
      height_factor <- c(1, 1.75)[input$show_fl + 1]
      pdf(file, width = 1.92, height = 1.44*height_factor, pointsize = 8 )
      print( plot_singleInput(cex_lab = 1, cex_axis = 1, lwd = 0.8, 
  ...
  # The current plot of an individual UV profile deconvolution can be downloaded.
  output$downloadPlot_decon <- downloadHandler(
    filename = function(){ 
      gsub(".csv|.txt|.pks|.tsv|.asc", "_QuAPProDeconv.pdf", as.character(input$select2) )
    },
    content = function(file) {
      height_factor <- c(1, 1.75)[input$show_deriv + 1]
      pdf(file, width = 1.92, height = 1.44*height_factor, pointsize = 8 )
      print( plot_deconv(cex_lab = 1, cex_axis = 1, lwd = 0.8, 
                              mgp1 = c(1.7, 0.5, 0), mgp2 = c(2, 0.8, 0), 
                              mar_factor = 0.6, lwd_box = 0.5) )
      dev.off()
    })  
  ...
  # After deconvolution, a table with the parameters of the model 
  # can be downloaded as csv file.
  output$downloadParam <- downloadHandler(
    filename = function(){ 
      gsub(".csv|.txt|.pks|.tsv|.asc", "_QuAPProPeakParam.csv", as.character(input$select2) )
    },
    content = function(file) {
      write.table(paste("# MSE =", 
                        round( val$mse[[input$select2]], digits = 3 ) ),
                  file, row.names = FALSE, col.names = FALSE, quote = FALSE)
      write.table(val$decon_param_df, file, row.names = FALSE,
                append = TRUE, sep = ",")
    }
  )
  
  # Download of the table showing the values in the bar plot:
  output$download_barplot_table <- downloadHandler(
    filename = "QuAPProBarplotTable.csv",
    content = function(file) {
      write.csv( val$barplot_table, file, row.names = TRUE)
    }
  )
  
  # Download of the bar plot as PDF file:
  output$download_barplot <- downloadHandler(
    filename = "QuAPProBarplot.pdf",
    content = function(file){
      pdf(file, width = 1.92, height = 1.24, pointsize = 8 )
      print(barplot(8, 1))
      dev.off()
    })
  ...
  # A notification is shown when a user activates deconvolution, but start and end of the 
...
# Tour of the Alignment/Areas tab:
guide1 <- Cicerone$
  new(allow_close = FALSE)$
  step(
    el = "input_data_wrapper",
    title = "File import",
    description = "Here, you can browse for your files. 
    You can also import previous analyses that you have exported using the 'Export Analysis' button.
    For loading an example analysis, press 'Example data'."
  )$
  step(
    el = "type_wrapper",
    title = "File Type",
    description = "If you want to import your own profiles, you need to choose the correct file type here. 
    Alternatively, you can specify the format of your input text file yourself
    by adjusting the column delimiter, decimal separator and the column number of the data.
    A number of lines can be skipped from the beginning of the file."
  )$
  step(
    el = "smooth_pol",
    title = "Smoothing",
    description = "If necessary, you can smooth your UV profile here."
  )$
  step(
    el = "baseline",
    title = "Setting a Baseline",
    description = "First click on this button and then into the plot to set a baseline for your UV profile."
  )$
  step(
    el = "x_anchor",
    title = "Aligning Profiles",
    description = "For aligning your profile to other profiles, click here and then into the plot
    to select a reference point for alignment along the x-axis ('x-anchor')."
  )$
  step(
    el = "helper_functions",
    title = "Peaks and Valleys",
    description = "If this option is ticked, the closest local maximum or minimum is automatically selected
    when you click into the plot to set an x-anchor (or the start/end of areas to quantify). 
    In addition, you can select the closest inflection point by ticking 'Inflection detection'"
    )$
  step(
    el = "show_fl",
    title = "Fluorescence Profiles",
    description = "In case your data contains a fluorescence profile in addition to UV absorbance,
    you can display it by setting a tick here. "
    )$
  step(
    el = "file_start",
    title = "Quantification of Areas",
    description = "Click here and then into the plot to select where the area to be quantified should start."
    )$
  step(
    el = "file_end",
    title = "Quantification of Areas",
    description = "Click here and then into the plot to select where the area to be quantified should end."
    )$
  step(
    el = "select_area_wrapper",
    title = "Quantification of Areas",
    description = "Choose an adequate name for your area. If you would like to add a name to the menu,
    just enter it into the text field and press 'Add ...'"
    )$
  step(
    el = "quantify_area",
    title = "Quantification of Areas",
    description = "Once you have selected an area and a name, click this button. 
    The result of the quantification will be displayed in the 'Quantification summary' tab."
    )$
  step(
    el = "select_alignment_wrapper",
    title = "Modifying Aligned Profiles",
    description = "Once you have set a baseline and an x-anchor for a profile, 
    it will automatically appear in the alignment section. You can change its appearance 
    like colors, lines and the order of aligned profiles."
    )$
  step(
    el = "new_name",
    title = "Naming Profiles",
    description = "By default, the profile name is used for the legend. 
    You can enter a better name here and apply it by clicking 'Rename'"
    )$
  step(
    el = "normalize_height",
    title = "Normalization of profiles",
    description = "The total area of a profile might vary due to technical reasons.
    To compensate for differences, you can tick this option. 
    It is essential that you have quantified an area with the name 'Total' for each profile in the alignment."
    )$
  step(
    el = "normalize_length",
    title = "Normalization of profiles",
    description = "If the length of profiles differs due to variations of elution speed between runs,
    you can tick this option."
    )$
  step(
    el = "show_fl_al",
    title = "Alignment of fluorescence profiles",
    description = "If your data contains fluorescence profiles, 
    they will be aligned along the same x-anchor. 
    The baseline of fluorescence profiles, however, have to be set separately.
    You can choose to display aligned fluorescence profiles here."
    )$
  step(
    el = "export",
    title = "Exporting your analysis",
    description = "You can export your entire analysis as .RData here, 
    and re-import it (via 'Browse') when you would like to add or modify something.")

# Tour of the Deconvolution tab:
guide2 <- Cicerone$
  new(allow_close = FALSE)$
  step(
    el = "select2_wrapper",
    title = "Available Profiles",
    description = "Once you have set a baseline for a profile in the first tab, it is available here for deconvolution."
    )$
  step(
    el = "smooth_pol_decon",
    title = "Settings for Peak Identification",
    description = "Peaks in polysome profiles often overlap strongly. Therefore, the negative second derivative minima 
    are used instead of local maxima to identify peak positions. 
    The profile itself and the second derivative need to be smoothed to some degree for this purpose.
    Play around with this parameter until the dashed grey lines reflect the peak positions that you would like to use for deconvolution."
  )$
  step(
    el = "resolution",
    title = "Settings for Peak Identification",
    description = "Local minima of the second derivative are identified 
    as a specific number of consecutively decreasing and then increasing points."
  )$
  step(
    el = "decon_start",
    title = "Region for Deconvolution",
    description = "Due to the high overlap of peaks in polysome profiles and the asymmetry of some peaks,
    deconvolution of the entire profile is usually not possible. 
    In profiles from mammalian cell lines, it typicall works well for the 40S, 60S and 80S peak.
    First press this button and then click into the profile
    to select the start of the region that you would like to consider for deconvolution."
  )$
  step(
    el = "helper_functions_decon",
    title = "Automatic recognition of peaks/valleys",
    description = "If this option is ticked, the closest local maximum or minimum is automatically selected
    when you click into the plot. 
    In addition, you can select the closest inflection point or negative second derivative minimum."
  )$
  step(
    el = "decon_end",
    title = "Region for Deconvolution",
    description = "Press this button and then click into the profile
    to select the end of the region that you would like to deconvolute."
  )$
  step(
    el = "decon_go",
    title = "Start Local deconvolution",
    description = "Once you selected your region of interest, click here to start the local deconvolution."
  )$
  step(
    el = "select_peak_wrapper",
    title = "Quantification of Peaks",
    description = "Choose an adequate name for your peak. If you would like to add a name to the menu,
    just enter it into the text field and press 'Add ...'"
  )$
  step(
    el = "quantify_peak",
    title = "Quantification of Peaks",
    description = "After successful deconvolution, you can click into the plot to select a peak for quantification.
    Active peaks will turn green. Click here to quantify the peak. The result will be shown in the table of the
    'Quantification Summary' tab."
  )
  
# Tour of the Bar Plot tab:
guide3 <- Cicerone$
  new(allow_close = FALSE)$
  step(
    el = "input_table_barplot_wrapper",
    title = "Select Values for Bar Plot",
    description = "Here, you can select which areas and peaks that you have quantified should be displayed as bar plot."
    )$
  step(
    el = "normalize_barplot_wrapper",
    title = "Select a Normalization",
    description = "The selected values can be normalized to display ratios instead of absolute values.
    Often, the proportion of polysomal ribosomes is used as a measure of global translation.
    In this case, you would select 'Polysomes_area' as values and normalize to 'Total_area'."
    )$
  step(
    el = "barplot_table",
    title = "Setting Conditions",
    description = "In this table, you can enter the conditions of the respective profiles,
    so that mean and standard deviation of values from the same condition can be shown in the bar plot.
    A second condition can be entered in the column Condition 2, if there are two variables
    (e.g. treatment and genotype)."
    )$
  step(
    el = "selected_cond1_wrapper",
       title = "Modifying the Appearance of the Bar Plot",
       description = "Conditions from the column Condition 1 can be selected here to change the colours of the bars,
       data points and error bars. Changes will only be applied as long as there is no Condition 2."
    )$
  step(
    el = "selected_cond2_wrapper",
    title = "Modifying the Appearance of the Bar Plot",
    description = "If there is a Condition 2, colours can be adjusted here. The colours of Condition 1 will be ignored."
  )
