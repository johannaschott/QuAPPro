...
# This is version 0.1.0 of QuAPPro, a shiny app for 
...
ui <- fluidPage(
  ...
  tabsetPanel(
                      ...
                      fluidRow(
                        column(6, 
                               selectInput("type", "Profile type", 
                                           choices = c("PeakTrak", "TRIAX (UV)", "TRIAX (+ Fl.)", "Custom"),
                                           selected = "PeakTrak",
                                           width = '100%')
                        ),
                        column(3,
    ...
    tabPanel(tags$strong("QuAPPro Manual"), icon = icon("question-circle"),
             fluidRow(
               column(2),
               column(8, htmltools::includeMarkdown("QuAPPro_v0-1-0_manual.Rmd")),
               column(2)
             ) 
    ),
    ...
    tabPanel(tags$strong("Release Notes", style = "color:blue"), icon = icon("info", style = "color:blue"), 
             ...
             tags$div("This is version QuAPPro v0.1.0. 
...
server <- function(input, output, session) {
  ...
  observeEvent(input$quantify_area,{
    ...
    val$sum_areas[[input$select_area]][input$select] <-
      sum( polysomes()[x_first:x_last] - val$baseline[input$select] )
    ...
    if(!( input$select_area %in% val$area_choices) ){
      val$area_choices <- c("Total", "80S", "Polysomes", "40S", "60S", input$select_area)
    }
  })
  ...
  ymax_all_fl <- reactive({
    ...
    profile_heights <- sapply(values_fluorescence()[f], FUN = max)
    ceiling( max( unlist( profile_heights ) ) )
  })
  
  ymin_all_fl <- reactive({
    f <- intersect(val$files_to_plot, names(val$baseline_fl) )
    profile_mins <- sapply(values_fluorescence()[f], FUN = min)
  ...
  plot_alignment <- function(cex_lab, cex_axis, lwd, 
                             ...
                             mar_factor, lwd_box, legend_width){
  ...
  output$plot_align <- renderPlot({
    ...
    if(length(val$files_to_plot) >= 1){
      plot_alignment(cex_lab = 1.6, cex_axis = 1.6, lwd = 2, 
                     ...
                     mar_factor = 1, lwd_box = 1, legend_width = 0.2)
    }
  })
  ...
  output$downloadPlot_single <- downloadHandler(
    filename = function(){ 
      gsub(".csv|.txt|.pks|.tsv", ".pdf", as.character(input$select) )
    },
  ...
  output$downloadPlot_aligned <- downloadHandler(
    ...
    content = function(file) {
      ...
      pdf(file, width = 2.7, height = 1.44*height_factor, pointsize = 8 )
      print( plot_alignment(cex_lab = 1, cex_axis = 1, lwd = 0.6, 
                            mgp1 = c(1.7, 0.5, 0), mgp2 = c(2, 0.8, 0),  
                            cex_legend = 1, lwd_factor = 0.4,
                            mar_factor = 0.6, lwd_box = 0.5, legend_width = 0.3 ) )
      dev.off()
    })  
