...
# This is version 0.3.1 of QuAPPro, a shiny app for 
  ...
  tabsetPanel(
    ...
    tabPanel(tags$strong("Alignment/Areas"), icon = icon("chart-area"),
             fluidRow(column(2, tags$h4(tags$strong("File Import")), 
                      ...
                      column(2, tags$h4(tags$strong("Take a Tour")) )
             ),
             fluidRow(
               ...
               column(2, style = "padding-left:20px",
                        ...
                        column(6, 
                               div(
                                 ...
                                 selectInput("type", "Profile type", 
                                             choices = c("PeakTrak", "TRIAX (UV)", "TRIAX (+ Fl.)", "PrimeView", "Custom"),
                                             selected = "PeakTrak",
                                             width = '100%')                               )
                      ...
                      fluidRow(
                        column(12, style = "padding-left:15px;padding-top:0px;padding-right:15px",
                               selectInput("select", "Select Profile", choices = c(), width = '100%')
                        )
                      ),  
               ...
               column(2, style = "padding-right:22px",
                      ...
                      fluidRow( style = "padding-top:10px;padding-bottom:10px",
                        column(6, 
                               actionButton("example1", label = "Example Data", icon = icon("chart-area"),
                                            width = "100%")
                        ),
                        column(6,
                               actionButton("tour1", label = "Guided Tour", icon = icon("circle-info"),
                                            width = "100%")
                        )
                      ),
             ...
             fluidRow(
               column(6,
                      fluidRow(  style = "padding-left:20px;padding-top:0px",
                        tags$img(src='logo_umm.svg', align = "left", width = '25%', height = '25%')
                      )
               ),
               column(6 ,style = "padding-left:0px",
                      tags$img(src='Logo-MI3-RGB.png', align = "right", width = '25%', height = '25%'))
             ),
             fluidRow(  style = "padding-left:20px;padding-top:0px",
                        tags$h6("QuAPPro is available free of charge under the GNU General Public License v3.0.",
                                align = "left")
             )
    ),
    ...
    tabPanel(tags$strong("Deconvolution"), icon = icon("cut"),
             fluidRow(column(2, tags$h4(tags$strong("UV Profile")), 
                             style = "padding-left:20px"),
                      column(8, tags$h4(tags$strong("Selected Profile") ) ),
                      column(2, tags$h4(tags$strong("Take a Tour") ) )
             ),
             fluidRow(
               # Section for uploading and selecting files:
                      ...
                      fluidRow(
                        column(12, style = "padding-left:15px;padding-top:0px;padding-right:15px",
                               div(id = "select2_wrapper",
                                   selectInput("select2", "Select Profile", choices = c(), width = '100%')
                               )
                        )
                      ), 
               ...
               column(2, style = "padding-right:22px",
                      ...
                      fluidRow( style = "padding-top:10px;padding-bottom:10px",
                                column(6, 
                                       actionButton("example2", label = "Example Data", icon = icon("chart-area"),
                                                    width = "100%")
                                ),
                                column(6,
                                       actionButton("tour2", label = "Guided Tour", icon = icon("circle-info"),
                                                    width = "100%")
                                )
                      ),
    ...
    tabPanel(tags$strong("Bar Plot"), icon = icon("chart-bar"),
             fluidRow(column(4, tags$h4(tags$strong("Feature Selection") ) ), 
                      column(6, tags$h4(tags$strong("Bar Plot") ) ),
                      column(2, tags$h4(tags$strong("Take a Tour")))
             ),
             fluidRow(
               column(4,
                               ...
                               fluidRow(
                                 column(6, 
                                        div( id = "input_table_barplot_wrapper",
                                          selectInput("input_table_barplot", label = "Select Values", choices = NULL)
                                        )
                                 ),
                                 column(6,
                               ...
                               fluidRow(
                                 column(2, 
                                        div( id = "selected_cond1_wrapper",
                                          selectInput("selected_cond1", label = "Select Condition 1", choices = NULL) 
                                        )
                                 ),
                                 column(2, colourInput("color_cond1", "Bars", palette = "square", value = "#858181")
                                 ),
                                 column(2, colourInput("color_dots_cond1", "Data points", palette = "square", value = "#000000")
                                 ),
                                 column(6)
                               ),
                               fluidRow(
                                 column(2, 
                                        div( id = "selected_cond2_wrapper",
                                          selectInput("selected_cond2", label = "Select Condition 2", choices = NULL) 
                                        )
                                 ),
                                 column(2, colourInput("color_cond2", "Bars", palette = "square", value = "#858181")
                                 ),
                                 column(2, colourInput("color_dots_cond2", "Data points", palette = "square", value = "#000000")
                                 ),
                                 column(6)
                               )
                        )
    ...
    tabPanel(tags$strong("QuAPPro Manual"), icon = icon("question-circle"),
             fluidRow(
               column(1),
               column(10, htmltools::includeMarkdown("QuAPPro_v0-3-1_manual_241220.Rmd")),
               column(1)
             ) 
    ),
    ...
    tabPanel(tags$strong("Release Notes", style = "color:blue"), icon = icon("info", style = "color:blue"), 
             fluidRow(
               column(1),
               column(10, htmltools::includeMarkdown("QuAPPro_v0-3-1_releaseNotes_241220.Rmd")),
               column(1)
             ) 
    ),
...
server <- function(input, output, session) {
  ...
  import <- function(path){
    ...
    updateSelectInput(session, "input_table_barplot", label = "Select Values", choices = unique(names(val$sum_areas) ), selected = val$selected_input_table_barplot)
  ...
  observeEvent(input$click_deconv$x, {
    ...
    # When deconvolution was successful, val$buttons is set to eight.
  ...
  observeEvent(input$decon_go, {
    ...
    if( any( class(decon_attempt) == "try-error") ){
      ...
      # The button value is set to 8, as a signal that deconvolution
      ...
      val$mse[[input$select2]] <- sum( ( df$y - val$model[[input$select2]][from:to] )^2 )/length(df$y)
 ...
 # An editable table is generated so that users can enter the conditions of their profiles.
  ...
  observeEvent(c(val$selected_input_table_barplot, val$normalize_barplot, val$df_quant), {
    ...
    barplot_input_name <- gsub("_area", "", val$selected_input_table_barplot)
    barplot_normalize_name <- gsub("_area", "", val$normalize_barplot)
  ...
  observeEvent(val$barplot_table, {
    ...
    if( !all(current_cond1 %in% val$cond1 ) ){
  ...
  output$download_barplot_table <- downloadHandler(
    ...
    content = function(file) {
      write.csv( val$barplot_table, file, row.names = F)
    }
  )
...
guide1 <- Cicerone$
  ...
  step(
    ...
    title = "File import",
    description = "Here, you can browse for your files. We already loaded two example profiles for you to play with."
  )$
  step(
    el = "type_wrapper",
    title = "File Type",
...
guide2 <- Cicerone$
  ...
  step(
    ...
    description = "Peaks in polysome profiles often overlap strongly. Therefore, the negative second derivativ minima 
  ...
  step(
    el = "decon_param",
    title = "Parameters of Peak Models",
    description = "Peaks are modeled as Gaussian distributions by non-linear regression. The parameters of the peak models
      are shown here."
  )$
  step(
    el = "MSE",
    title = "Mean Squared Error",
    description = "The Mean Squared Error (MSE) indicates how well the model fits the profile data in the region selected for 
      deconvolution. The lower the MSE, the better is the model."
  )
